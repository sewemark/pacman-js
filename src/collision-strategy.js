export default function PacmanCollisionStartegy(mapHeight, mapWidth) {
  ...
  this.destinationCheker = {
    37: (playerPosition) => {
        console.log( this.map[playerPosition.y][playerPosition.x-1]);
      return (playerPosition.x - 1 >= 0 || playerPosition.x - 1 < mapWidth) && this.map[playerPosition.y][playerPosition.x-1] != 1},
  ...
  this.mapUpdater = {
    37: (map, playerPosition) => {
      map[playerPosition.y][playerPosition.x] = 0;
      map[playerPosition.y][playerPosition.x - 1] = 2;
    },
    39: (map,playerPosition) => {
      map[playerPosition.y][playerPosition.x] = 0;
      map[playerPosition.y][playerPosition.x + 1] = 2;
    },
    38: (map,playerPosition) => {
      map[playerPosition.y][playerPosition.x] = 0;
      map[playerPosition.y - 1][playerPosition.x] = 2;
    },
    40: (map, playerPosition) => {
      map[playerPosition.y][playerPosition.x] = 0;
      map[playerPosition.y + 1][playerPosition.x] = 2;
    },
  }
  ...
  this.updateMap = function(destinationDirection, map, playerPosition) {
    return this.mapUpdater[destinationDirection](map, playerPosition);
